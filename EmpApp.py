...
def attendance():
    msg=" "
    if request.method == 'POST' and 'emp_id' and 'an_leave' and 'med_leave' and 'un_leave' in request.form:
        emp_id=request.form['emp_id']
        an_leave=request.form['an_leave']
        med_leave=request.form['med_leave']
        un_leave=request.form['un_leave']
        cursor = db_conn.cursor()
        cursor.execute('SELECT * FROM employee WHERE emp_id = %s', (emp_id))
        user = cursor.fetchone()
        db_conn.commit()
